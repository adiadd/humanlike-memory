      ...
      >
        <ComboboxPrimitive.Popup
          ...
          className={cn(
            popupContainer,
            popupAnimation,
            '*:data-[slot=input-group]:bg-input/30 *:data-[slot=input-group]:border-input/30 max-h-72 min-w-36 overflow-hidden duration-100 *:data-[slot=input-group]:m-1 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-8 *:data-[slot=input-group]:shadow-none group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) data-[chips=true]:min-w-(--anchor-width)',
            className,
          )}
          {...props}
        />
...
function ComboboxChips({
  ...
  return (
    <ComboboxPrimitive.Chips
      ...
      className={cn(
        inputBase,
        // Focus ring uses focus-within for container focus behavior
        'focus-within:border-ring focus-within:ring-ring/50 focus-within:ring-1',
        // Invalid states use has-aria-invalid for child element validation
        ...
        'flex min-h-8 flex-wrap items-center gap-1 bg-clip-padding px-2.5 py-1 text-xs transition-colors has-data-[slot=combobox-chip]:px-1',
        className,
      )}
      {...props}
    />
